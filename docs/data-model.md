...
상품명은 실제 쇼핑몰에서 볼 법한 형태로 작성했으며 가격·재고·평점은 검색 조건 검증을 위해 구성한 합성 값이다.

| 문서 | 포함/제외/경계 역할 | 연결 질문 | 예상과 field 근거 |
|---|---|---|---|
| `P-0001` | 정상 포함 | Q1, Q2, Q3 | `prd_nm`에 무선 이어폰, 재고 있음, 전자기기, 가격 129,000원, 평점 4.7 |
| `P-0002` | 포함 경계 | Q2, Q3 | 가격 200,000원과 평점 4.5로 두 조건의 포함 경계 |
| `P-0003` | 제외 | Q1, Q2, Q3 | 유선 상품, 재고 없음, 가격 29,900원, 평점 4.2 |
...
| `prd_nm` | `Soundcore Liberty 4 NC 무선 이어폰` | 전문 검색·표시 | `text` | Q1 | 상품명을 분석한 token으로 검색 |
| `in_stock` | `true` | 정확 조건·표시 | `boolean` | Q2 | 재고 있음과 없음을 두 상태로 구분 |
| `category` | `전자기기` | 정확 조건·집계·표시 | `keyword` | Q2 | 카테고리 전체 값으로 필터하고 집계 |
| `price` | `129000` | 범위·정렬·표시·집계 | `integer` | Q2 | 정수 가격의 범위 계산과 숫자 정렬 |
