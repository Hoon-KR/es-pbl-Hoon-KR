# Day 2 v1 학생교재 — 데이터 모델링과 적재

2026-09-01 동기화판. 69장 PPT · ES/Kibana 9.5.0 · Windows 11.
[시작 안내](v1/README.md) · [실습/제출 확인표](v1/PRACTICE_GUIDE.md) · [공통 요청](v1/lecture-requests.http) · [생성 가이드](v1/DATA_GENERATION_GUIDE.md) · [작성 양식](v1/templates/README.md)
...
Console에서 다음을 하나씩 실행한다. [공통 파일](v1/lecture-requests.http)의 T09 부분도 사용할 수 있다.
...
[공통 요청](v1/lecture-requests.http)의 S39와 S40을 사용한다.
...
[공통 요청](v1/lecture-requests.http)의 T14 요청을 한 개씩 실행한다. 구 requests/03-analyze-and-crud.http 또는 v2 workshop은 사용하지 않는다.
...
loader 오류가 없는지 확인한 뒤 [공통 요청](v1/lecture-requests.http)의 S58 _count와 분포를 Console에서 실행한다. 빈 기준 index에 고유10000건만 넣었다면 count10000·8category각1250이다. 실제 재고 비율, price stats, created_at min/max를 evidence/day-02-data.md에 기록한다. Console 결과는 JSON이며 슬라이드 막대는 설명용 예다. errors:false만으로 품질이나 총 고유건수가 보증되지 않는다.
...
4. [개인 생성 가이드](v1/DATA_GENERATION_GUIDE.md)의 mapping 검증 포함 명령으로 생성한다. 평면 field와 tags 배열이 기본 지원이다. 대표3건을 고정 사례로 넣는 옵션도 가이드에 있다.
...
[공통 요청](v1/lecture-requests.http)의 S63 GET으로 product-cleanup 존재를 확인한다. 없을 때만 PUT한다. 다른 정의가 있으면 덮어쓰지 않고 새 이름으로 정의/후속 경로를 함께 바꾼다. products의 default_pipeline은 설정하지 않는다.
...
[제출 확인표](v1/PRACTICE_GUIDE.md)의 전체 파일을 점검한다. 대표3건 sample-documents.json과 생성기30건 표본은 다르므로 모두 제출한다. 전체 대량 NDJSON은 제외하고 설정·코드·표본·요약으로 재현한다. README에는 실행 순서와 문서 링크를 적는다.
