...
$bulkPath = Join-Path $OutputDir 'products-10000.ndjson'
$samplePath = Join-Path $OutputDir 'products-sample-30.ndjson'
...
try {
  for ($number = 1; $number -le $Count; $number++) {
    ...
    $created = $start.AddSeconds($rng.NextDouble() * ($end - $start).TotalSeconds)
...
$summary = [ordered]@{
  ...
  seed = $Seed
  files = [ordered]@{ bulk = 'products-10000.ndjson'; sample = 'products-sample-30.ndjson' }
